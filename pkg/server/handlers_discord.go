...
// linkDiscordUserAdmin allows an admin to link any Discord user to any LDAP account
func (c *Config) linkDiscordUserAdmin(ctx *gin.Context) {
	utils.DebugLog("API: Admin request to link Discord user to LDAP")
	...
	if err := ctx.ShouldBindJSON(&req); err != nil {
		utils.ErrorLog("API: Invalid admin Discord link request: %v", err)
		ctx.JSON(http.StatusBadRequest, types.APIResponse{
			Success: false,
			Error:   "Invalid request: " + err.Error(),
		})
	...
	utils.DebugLog("API: Admin linking Discord ID %s (%s) to LDAP user %s", req.DiscordID, req.DiscordName, req.LDAPUser)

	if c.db == nil {
		utils.ErrorLog("Database is nil in linkDiscordUserAdmin")
		ctx.JSON(http.StatusInternalServerError, types.APIResponse{
			Success: false,
			Error:   "Database not initialized",
		})
	...
	if err := c.db.LinkDiscordToLDAPAdmin(req.DiscordID, req.DiscordName, req.LDAPUser); err != nil {
		utils.ErrorLog("API: Failed to link Discord to LDAP (admin): %v", err)
		ctx.JSON(http.StatusInternalServerError, types.APIResponse{
			Success: false,
			Error:   "Failed to link accounts: " + err.Error(),
		})
	...
	utils.InfoLog("Successfully linked Discord ID %s (%s) to LDAP user %s (admin)", req.DiscordID, req.DiscordName, req.LDAPUser)

	ctx.JSON(http.StatusOK, types.APIResponse{
		Success: true,
		Message: "Discord account linked successfully (admin)",
		Data: map[string]string{
			"discord_id":   req.DiscordID,
			"discord_name": req.DiscordName,
			"ldap_user":    req.LDAPUser,
