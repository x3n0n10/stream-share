...
func (b *Bot) handleStatus(s *discordgo.Session, m *discordgo.MessageCreate, _ []string) {
	...
	if err != nil || !ok {
		b.fail(m.ChannelID, "❌ Status Failed", fmt.Sprintf("Failed to get status: %v", err))
		return
	}
	mp, _ := data.(map[string]interface{})
	streams := 0
	if v, ok := mp["streams_count"].(float64); ok {
		streams = int(v)
	}
	users := 0
	if v, ok := mp["users_count_active"].(float64); ok {
		users = int(v)
	}
	text := ""
	if sstr, ok := mp["text"].(string); ok {
		text = strings.TrimSpace(sstr)
	}
