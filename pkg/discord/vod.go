...
func (b *Bot) handleVOD(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	...
	if len(tokens) > 0 {
		utils.DebugLog("Discord: filter tokens=%v season=%d episode=%d", tokens, fSeason, fEpisode)
	}
	if len(tokens) > 0 {
			...
			if leftSecs > 0 {
				...
				if days >= 1 {
					...
					if hours <= 1 {
						...
						left = fmt.Sprintf("%d hours", hours)
					}
				}
			}
