...
func parseQueryFilters(q string) (tokens []string, season, episode int) {
	...
	if m := regexp.MustCompile(`(?i)\bs(\d{1,2})e(\d{1,2})\b`).FindStringSubmatch(s); m != nil {
		...
		if m := regexp.MustCompile(`(?i)\bs(\d{1,2})\b`).FindStringSubmatch(s); m != nil {
			season = atoiSafe(m[1])
		}
		if m := regexp.MustCompile(`(?i)\be(\d{1,2})\b`).FindStringSubmatch(s); m != nil {
			episode = atoiSafe(m[1])
		}
	}
...
func filterVODResults(results []types.VODResult, tokens []string, season, episode int) []types.VODResult {
	...
	for _, r := range results {
		...
		parts := []string{}
		if r.SeriesTitle != "" {
			parts = append(parts, r.SeriesTitle)
		}
		if r.Title != "" {
			parts = append(parts, r.Title)
		}
		if r.EpisodeTitle != "" {
			parts = append(parts, r.EpisodeTitle)
		}
		if r.Category != "" {
