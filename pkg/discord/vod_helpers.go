...
func buildDescriptionForVOD(r types.VODResult) string {
	parts := []string{}
	if r.StreamType != "" {
		parts = append(parts, asciiTitle(r.StreamType))
	}
	if r.Category != "" {
...
func toVODResults(arr []interface{}) []types.VODResult {
	...
	for _, it := range arr {
		...
		if v, ok := rm["Season"].(float64); ok {
			vr.Season = int(v)
		}
		if v, ok := rm["Episode"].(float64); ok {
			vr.Episode = int(v)
		}
...
func buildOptionsForRange(results []types.VODResult, start, end int) []discordgo.SelectMenuOption {
	...
	for i := start; i < end; i++ {
		...
		if r.Size != "" {
			label = fmt.Sprintf("%s — %s", label, r.Size)
		}
